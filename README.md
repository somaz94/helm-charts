...
| [prometheus-cr](charts/prometheus-cr) | Custom resources for the Prometheus Operator — data-driven `ServiceMonitor` / `PodMonitor` / `PrometheusRule` / `Probe` (monitoring.coreos.com/v1) + `ScrapeConfig` (v1alpha1) rendered from list-valued inputs, with per-entry `namespace` override, shared `commonLabels` / `commonAnnotations`, and a per-entry `specExtra` escape hatch. Closes the gap where kube-prometheus-stack / prometheus-operator installs the CRDs but leaves app-level monitoring CRs to each consumer. Safe empty defaults (no-op install). |
| [metallb-cr](charts/metallb-cr) | Custom resources for MetalLB — data-driven `IPAddressPool`, `L2Advertisement`, `BGPAdvertisement`, `BGPPeer` (metallb.io/v1beta2), `Community`, `BFDProfile` rendered from list-valued inputs with shared `commonLabels` / `commonAnnotations` and per-entry `specExtra` escape hatch. Closes the gap where the upstream `metallb/metallb` chart intentionally does not template config CRs (replaces the out-of-band `kubectl apply` hook). Safe empty defaults (no-op install). |
| [karpenter-cr](charts/karpenter-cr) | Custom resources for AWS Karpenter — data-driven `EC2NodeClass` + `NodePool` (multi-pool via `nodeClasses[]` / `nodePools[]` lists, shared defaults with per-entry override). Discovery-tag subnet / SG selection, secure-by-default (IMDSv2, encrypted gp3); AMI selection required (no silent drift). Karpenter v1 API, self-managed controller or EKS Auto Mode (`nodeClassGroup` / `nodeClassKind`). |
| [aws-storageclass](charts/aws-storageclass) | Data-driven AWS `StorageClass` objects — EBS (`gp3`/`gp2`/`io2`) and EFS — rendered from a single `storageClasses[]` list, each entry individually toggleable via `enabled`. Per-entry `isDefault` (is-default-class), `parameters`, `volumeBindingMode`, `reclaimPolicy`, `allowVolumeExpansion`, `mountOptions`, `allowedTopologies`, shared `commonLabels` / `commonAnnotations`, and a `parametersExtra` escape hatch. Closes the gap where the AWS CSI drivers install the provisioner but do not create StorageClasses. Safe empty defaults (no-op install). |
...
| [external-secrets-cr](charts/external-secrets-cr) | Custom resources for the External Secrets Operator — data-driven `SecretStore` / `ClusterSecretStore` / `ExternalSecret` / `ClusterExternalSecret` (external-secrets.io/v1) + `PushSecret` (v1alpha1) rendered from list-valued inputs, with per-entry `namespace` override on namespaced kinds, verbatim provider pass-through, shared `commonLabels` / `commonAnnotations`, and a per-entry `specExtra` escape hatch. Closes the gap where the upstream external-secrets chart installs the operator + CRDs but does not template config CRs. Safe empty defaults (no-op install). |
...
| [redis](charts/redis) | Standalone single-node Redis — Deployment + PVC + Service, optional Secret (auth password), optional ConfigMap (`redis.conf`), NetworkPolicy, ServiceAccount, image pull Secret. `persistence.mountPath` + `persistence.existingClaim` + `fullnameOverride` for in-place migration off raw-YAML deployments (e.g. legacy `/redis-master-data` mount). |
...
| [buildkit](charts/buildkit) | Standalone BuildKit StatefulSet — `moby/buildkit` pod (rootful, privileged) with optional registry CA-bundle Secret + optional `buildkitd.toml` ConfigMap, cache PVC via `volumeClaimTemplates`, ClusterIP Service on TCP 1234. Auto-renders a minimal `buildkitd.toml` wiring the registry CA when `registry.caBundle.enabled` is true and no custom config is supplied. Suits in-cluster `docker buildx` consumers (kubernetes/remote driver) needing multi-arch builds against a self-signed registry where ephemeral buildx buildkit containers fail OAuth-token cert verification. |
